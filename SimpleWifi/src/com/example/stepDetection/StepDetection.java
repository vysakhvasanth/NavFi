...
/**
 * 
 * Open source project: footpath This class is fed with data from the
 * Accelerometer and Compass sensors. If a step is detected on the acc data it
 * calls the trigger function on its interface StepTrigger, with the given
 * direction.
 * 
 * Usage: Create an object: stepDetection = new StepDetection(this, this, a,
 * peak, step_timeout_ms);
 * 
 * @author Paul Smith
 * 
 */
public class StepDetection {
	...
	public StepDetection(Context context, StepTrigger st, double a,
			double peak, int step_timeout_ms) {
	...
	private void updateData() {
		...
		// Create local value for compass and old_z, such that it is consistent
		// during logs
		...
		if ((now_ms - last_step_ts) > step_timeout_ms && checkForStep(peak)) {
			...
			Log.i("FOOTPATH", "Detected step  in  round = " + round + " @ "
					+ now_ms);
	...
	private boolean checkForStep(double peakSize) {
		...
		double answer = values_history[(vhPointer - 1 + vhSize) % vhSize];

		for (int t = 1; t <= lookahead; t++) {
			if ((values_history[(vhPointer - 1 - t + vhSize + vhSize) % vhSize]
					- values_history[(vhPointer - 1 + vhSize) % vhSize] > diff)) {
				Log.i("FOOTPATH",
						"Detected step with t = "
								+ t
								+ ", diff = "
								+ diff
								+ " < "
								+ (values_history[(vhPointer - 1 - t + vhSize + vhSize)
										% vhSize] - values_history[(vhPointer - 1 + vhSize)
										% vhSize]));
				return true;
			}
		}
